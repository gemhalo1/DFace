...
def gen_data(anno_file, data_dir, prefix):
    ...
    for annotation in annotations:
        ...
        for i in range(10):
            ...
            nx1 = int(round(max(x1 + w / 2 - bbox_size / 2 + delta_x, 0)))
            ny1 = int(round(max(y1 + h / 2 - bbox_size / 2 + delta_y, 0)))

            nx2 = int(round(nx1 + bbox_size))
            ny2 = int(round(ny1 + bbox_size))
            if nx2 > width or ny2 > height:
                continue
            crop_box = np.array([nx1, ny1, nx2, ny2], dtype=np.int32)
            ...
            if iou > 0.65:
                ...
                f.write(save_file + ' -2 %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f \n' % \
                        (offset_x1, offset_y1, offset_x2, offset_y2,
                         offset_left_eye_x, offset_left_eye_y,
                         offset_right_eye_x, offset_right_eye_y,
                         offset_nose_x, offset_nose_y,
                         offset_left_mouth_x, offset_left_mouth_y,
